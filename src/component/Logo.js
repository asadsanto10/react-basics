...
import LogoStyles from '../assets/css/Logo.module.css';

const Logo = () => {
  const [colors, setColors] = useState('black');

  const randomColor = () => {
    const color = ['red', 'green', 'blue', 'alpha', 'orange'];
    return color[Math.floor(Math.random() * color.length)];
  };
  return (
    <div className={LogoStyles.logo}>
      <div className={LogoStyles.hair}>
      ...
      <div className={LogoStyles.glasses}>
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 103.63 42.68">
          <g>
            <g>
              <rect
                className={`${LogoStyles.coloredLine} border`}
                x="30.08"
                y="11.04"
                width="2.54"
                height="15.01"
      ...
      <div className={LogoStyles.play}>
        <button type="button" onClick={() => setColors(randomColor())}>
          Colorize Glass
        </button>
        <button onClick={() => setColors('black')} type="button">
          Reset Glass
        </button>
      </div>
    </div>
